...
var require_dist = __commonJS({
  "node_modules/jsdoc-type-pratt-parser/dist/index.js"(exports, module) {
    (function(global, factory) {
      ...
      function parse2(expression, mode) {
        switch (mode) {
          case "closure":
      ...
      function tryParse2(expression, modes = ["typescript", "closure", "jsdoc"]) {
        let error;
        for (const mode of modes) {
          try {
            return parse2(expression, mode);
          } catch (e) {
            error = e;
          }
        }
      ...
      function transform2(rules2, parseResult) {
        const rule = rules2[parseResult.type];
        if (rule === void 0) {
          throw new Error(`In this set of transform rules exists no rule for type ${parseResult.type}.`);
        }
        return rule(parseResult, (aParseResult) => transform2(rules2, aParseResult));
      }
      ...
      function stringifyRules2() {
        return {
          JsdocTypeParenthesis: (result, transform3) => `(${result.element !== void 0 ? transform3(result.element) : ""})`,
          JsdocTypeKeyof: (result, transform3) => `keyof ${transform3(result.element)}`,
          JsdocTypeFunction: (result, transform3) => {
            if (!result.arrow) {
              let stringified = result.constructor ? "new" : "function";
              if (!result.parenthesis) {
                return stringified;
              }
              stringified += `(${result.parameters.map(transform3).join(", ")})`;
              if (result.returnType !== void 0) {
                stringified += `: ${transform3(result.returnType)}`;
              }
              return stringified;
            } else {
              if (result.returnType === void 0) {
              ...
              let stringified = `(${result.parameters.map(transform3).join(", ")}) => ${transform3(result.returnType)}`;
          ...
          JsdocTypeTuple: (result, transform3) => `[${result.elements.map(transform3).join(", ")}]`,
          JsdocTypeVariadic: (result, transform3) => result.meta.position === void 0 ? "..." : applyPosition(result.meta.position, transform3(result.element), "..."),
          JsdocTypeNamePath: (result, transform3) => {
            const left = transform3(result.left);
            const right = transform3(result.right);
          ...
          JsdocTypeAny: () => "*",
          JsdocTypeGeneric: (result, transform3) => {
            if (result.meta.brackets === "square") {
              const element = result.elements[0];
              const transformed = transform3(element);
              ...
              return `${transform3(result.left)}${result.meta.dot ? "." : ""}<${result.elements.map(transform3).join(", ")}>`;
            }
          },
          JsdocTypeImport: (result, transform3) => `import(${transform3(result.element)})`,
          JsdocTypeObjectField: (result, transform3) => {
            let text = "";
            if (result.readonly) {
              text += "readonly ";
            }
            ...
            if (result.right === void 0) {
              ...
              return text + `: ${transform3(result.right)}`;
            }
          },
          JsdocTypeJsdocObjectField: (result, transform3) => {
            return `${transform3(result.left)}: ${transform3(result.right)}`;
          },
          JsdocTypeKeyValue: (result, transform3) => {
            let text = result.key;
            if (result.optional) {
              text += "?";
            }
          ...
          JsdocTypeNotNullable: (result, transform3) => applyPosition(result.meta.position, transform3(result.element), "!"),
          JsdocTypeNull: () => "null",
          JsdocTypeNullable: (result, transform3) => applyPosition(result.meta.position, transform3(result.element), "?"),
          JsdocTypeNumber: (result) => result.value.toString(),
          JsdocTypeObject: (result, transform3) => `{${result.elements.map(transform3).join((result.meta.separator === "comma" ? "," : ";") + " ")}}`,
          JsdocTypeOptional: (result, transform3) => applyPosition(result.meta.position, transform3(result.element), "="),
          JsdocTypeSymbol: (result, transform3) => `${result.value}(${result.element !== void 0 ? transform3(result.element) : ""})`,
          JsdocTypeTypeof: (result, transform3) => `typeof ${transform3(result.element)}`,
          JsdocTypeUndefined: () => "undefined",
          JsdocTypeUnion: (result, transform3) => result.elements.map(transform3).join(" | "),
          JsdocTypeUnknown: () => "?",
          JsdocTypeIntersection: (result, transform3) => result.elements.map(transform3).join(" & "),
          JsdocTypeProperty: (result) => quote(result.value, result.meta.quote),
          JsdocTypePredicate: (result, transform3) => `${transform3(result.left)} is ${transform3(result.right)}`,
          JsdocTypeIndexSignature: (result, transform3) => `[${result.key}: ${transform3(result.right)}]`,
          JsdocTypeMappedType: (result, transform3) => `[${result.key} in ${transform3(result.right)}]`,
          JsdocTypeAsserts: (result, transform3) => `asserts ${transform3(result.left)} is ${transform3(result.right)}`
        };
      }
      const storedStringifyRules = stringifyRules2();
      function stringify2(result) {
        return transform2(storedStringifyRules, result);
      }
      const reservedWords = [
        "null",
        "true",
      ...
      const catharsisTransformRules = {
        JsdocTypeOptional: (result, transform3) => {
          const transformed = transform3(result.element);
          transformed.optional = true;
          return transformed;
        },
        JsdocTypeNullable: (result, transform3) => {
          const transformed = transform3(result.element);
          transformed.nullable = true;
          return transformed;
        },
        JsdocTypeNotNullable: (result, transform3) => {
          const transformed = transform3(result.element);
          transformed.nullable = false;
          return transformed;
        },
        JsdocTypeVariadic: (result, transform3) => {
        ...
        JsdocTypeFunction: (result, transform3) => {
          ...
          const transformed = {
            type: "FunctionType",
            params: params.params.map(transform3)
          };
          if (params.this !== void 0) {
            transformed.this = transform3(params.this);
          }
          if (params.new !== void 0) {
            transformed.new = transform3(params.new);
          }
          if (result.returnType !== void 0) {
            transformed.result = transform3(result.returnType);
          }
          return transformed;
        },
        JsdocTypeGeneric: (result, transform3) => ({
          type: "TypeApplication",
          applications: result.elements.map((o) => transform3(o)),
          expression: transform3(result.left)
        }),
        ...
        JsdocTypeObject: (result, transform3) => {
          const transformed = {
            type: "RecordType",
            fields: []
          };
          for (const field of result.elements) {
            if (field.type !== "JsdocTypeObjectField" && field.type !== "JsdocTypeJsdocObjectField") {
              transformed.fields.push({
                type: "FieldType",
                key: transform3(field),
                value: void 0
              });
            } else {
              transformed.fields.push(transform3(field));
            }
          }
          return transformed;
        },
        JsdocTypeObjectField: (result, transform3) => {
          ...
          return {
            ...
            value: result.right === void 0 ? void 0 : transform3(result.right)
          };
        },
        JsdocTypeJsdocObjectField: (result, transform3) => ({
          type: "FieldType",
          key: transform3(result.left),
          value: transform3(result.right)
        }),
        JsdocTypeUnion: (result, transform3) => ({
          type: "TypeUnion",
          elements: result.elements.map((e) => transform3(e))
        }),
        JsdocTypeKeyValue: (result, transform3) => {
          return {
            type: "FieldType",
        ...
        JsdocTypeNamePath: (result, transform3) => {
          const leftResult = transform3(result.left);
          let rightValue;
          if (result.right.type === "JsdocTypeSpecialNamePath") {
            rightValue = transform3(result.right).name;
          } else {
        ...
        JsdocTypeParenthesis: (result, transform3) => transform3(assertRootResult(result.element)),
      ...
      function catharsisTransform2(result) {
        return transform2(catharsisTransformRules, result);
      }
      ...
      const jtpRules = {
        JsdocTypeOptional: (result, transform3) => ({
          type: "OPTIONAL",
          value: transform3(result.element),
          meta: {
        ...
        JsdocTypeVariadic: (result, transform3) => {
          ...
          if (result.element !== void 0) {
            transformed.value = transform3(result.element);
          }
          return transformed;
        },
        JsdocTypeName: (result) => ({
        ...
        JsdocTypeTypeof: (result, transform3) => ({
          type: "TYPE_QUERY",
          name: transform3(result.element)
        }),
        JsdocTypeTuple: (result, transform3) => ({
          type: "TUPLE",
          entries: result.elements.map(transform3)
        }),
        JsdocTypeKeyof: (result, transform3) => ({
          type: "KEY_QUERY",
          value: transform3(result.element)
        }),
        ...
        JsdocTypeFunction: (result, transform3) => {
            ...
            params: specialParams.params.map((param) => {
              if (param.type === "JsdocTypeKeyValue") {
                ...
                return {
                  ...
                  name: param.key,
                  typeName: transform3(param.right)
                };
              } else {
                return transform3(param);
              }
            }),
            new: null,
            returns: null
          ...
          if (specialParams.this !== void 0) {
            transformed.this = transform3(specialParams.this);
          } else if (!result.arrow) {
            transformed.this = null;
          }
          if (specialParams.new !== void 0) {
            transformed.new = transform3(specialParams.new);
          }
          if (result.returnType !== void 0) {
            transformed.returns = transform3(result.returnType);
          }
          return transformed;
        },
        JsdocTypeGeneric: (result, transform3) => {
          const transformed = {
            type: "GENERIC",
            subject: transform3(result.left),
            objects: result.elements.map(transform3),
            meta: {
        ...
        JsdocTypeObjectField: (result, transform3) => {
          ...
          let right = transform3(result.right);
          if (result.optional) {
            right = {
              type: "OPTIONAL",
              value: right,
        ...
        JsdocTypeUnion: (result, transform3) => nestResults("UNION", result.elements.map(transform3)),
        JsdocTypeParenthesis: (result, transform3) => ({
          type: "PARENTHESIS",
          value: transform3(assertRootResult(result.element))
        }),
        JsdocTypeNull: () => ({
          type: "NAME",
          name: "null"
        ...
        JsdocTypeIntersection: (result, transform3) => nestResults("INTERSECTION", result.elements.map(transform3)),
      ...
      function jtpTransform2(result) {
        return transform2(jtpRules, result);
      }
      function identityTransformRules2() {
        return {
          JsdocTypeIntersection: (result, transform3) => ({
            type: "JsdocTypeIntersection",
            elements: result.elements.map(transform3)
          }),
          JsdocTypeGeneric: (result, transform3) => ({
            type: "JsdocTypeGeneric",
            left: transform3(result.left),
            elements: result.elements.map(transform3),
            meta: {
              dot: result.meta.dot,
          ...
          JsdocTypeTypeof: (result, transform3) => ({
            type: "JsdocTypeTypeof",
            element: transform3(result.element)
          }),
          JsdocTypeSymbol: (result, transform3) => {
            const transformed = {
              type: "JsdocTypeSymbol",
              value: result.value
            };
            if (result.element !== void 0) {
              transformed.element = transform3(result.element);
            }
            return transformed;
          },
          JsdocTypeOptional: (result, transform3) => ({
          ...
          JsdocTypeObjectField: (result, transform3) => ({
            type: "JsdocTypeObjectField",
            key: result.key,
            right: result.right === void 0 ? void 0 : transform3(result.right),
          ...
          JsdocTypeImport: (result, transform3) => ({
            type: "JsdocTypeImport",
            element: transform3(result.element)
          }),
          ...
          JsdocTypeFunction: (result, transform3) => {
            const transformed = {
              type: "JsdocTypeFunction",
              arrow: result.arrow,
              parameters: result.parameters.map(transform3),
              constructor: result.constructor,
              parenthesis: result.parenthesis
            };
            if (result.returnType !== void 0) {
              transformed.returnType = transform3(result.returnType);
            }
            return transformed;
          },
          JsdocTypeKeyof: (result, transform3) => ({
          ...
          JsdocTypePredicate: (result, transform3) => ({
            type: "JsdocTypePredicate",
            left: transform3(result.left),
            right: transform3(result.right)
          }),
          JsdocTypeIndexSignature: (result, transform3) => ({
            type: "JsdocTypeIndexSignature",
            key: result.key,
            right: transform3(result.right)
          }),
          JsdocTypeMappedType: (result, transform3) => ({
            type: "JsdocTypeMappedType",
            key: result.key,
            right: transform3(result.right)
          }),
          JsdocTypeAsserts: (result, transform3) => ({
            type: "JsdocTypeAsserts",
            left: transform3(result.left),
            right: transform3(result.right)
          })
      ...
      function _traverse(node, parentNode, property, onEnter, onLeave) {
        ...
        const keysToVisit = visitorKeys2[node.type];
      ...
      function traverse2(node, onEnter, onLeave) {
        _traverse(node, void 0, void 0, onEnter, onLeave);
      }
      exports2.catharsisTransform = catharsisTransform2;
      exports2.identityTransformRules = identityTransformRules2;
      exports2.jtpTransform = jtpTransform2;
      exports2.parse = parse2;
      exports2.stringify = stringify2;
      exports2.stringifyRules = stringifyRules2;
      exports2.transform = transform2;
      exports2.traverse = traverse2;
      exports2.tryParse = tryParse2;
      exports2.visitorKeys = visitorKeys2;
    });
  }
});

// commonjs-named-exports:jsdoc-type-pratt-parser
var import_dist = __toESM(require_dist());
import_dist.catharsisTransform;
import_dist.default;
import_dist.identityTransformRules;
import_dist.jtpTransform;
var export_parse = import_dist.parse;
import_dist.stringify;
var export_stringifyRules = import_dist.stringifyRules;
var export_transform = import_dist.transform;
import_dist.traverse;
import_dist.tryParse;
import_dist.visitorKeys;
...
o(He, "extractReturns");
var g = export_stringifyRules(), Qe = g.JsdocTypeObject;
g.JsdocTypeAny = () => "any";
...
function te(e) {
  try {
    return export_parse(e, "typescript");
  } catch {
    return null;
  }
}
...
var cn = /* @__PURE__ */ o((e) => {
  ...
  return i ? __STORYBOOK_MODULE_PREVIEW_API__.combineParameters(i, r) : r;
}, "enhanceArgTypes");

// src/docs-tools/shared.ts
var ye = "storybook/docs", yn = `${ye}/snippet-rendered`, gt = /* @__PURE__ */ ((n) => (n.AUTO = "auto", n.
CODE = "code", n.DYNAMIC = "dynamic", n))(gt || {});

export { __export as _, __esm as a, __toESM as b, cn as c, __commonJS as d, __toCommonJS as e, gt as g, yn as y };

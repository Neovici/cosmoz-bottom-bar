...
`,ld=Symbol("openMenu"),cd=e=>{const t=e.shadowRoot?.querySelector("#dropdown");if(!t||t.hasAttribute("hidden"))return;t.shadowRoot?.querySelector("cosmoz-dropdown")?.shadowRoot?.querySelector("#dropdownButton")?.click()},ud=e=>{const[t,i]=ie({visible:new Set,overflowing:new Set});te(()=>{e.dispatchEvent(new CustomEvent("reflow",{detail:t}))},[t]);const n=z(()=>[...t.visible,...t.overflowing],[t]),s=z(()=>n.map(l=>({element:l,priority:Number(l.dataset.priority??0),text:l.getAttribute("title")||l.textContent?.trim()||""})).toSorted((l,c)=>c.priority-l.priority),[n]),{maxToolbarItems:r=1}=e,o=Math.min(r,t.visible.size);te(()=>{s.forEach(({element:l,priority:c},u)=>{const d=u<o;l.toggleAttribute("processed",!0),l.dataset.visibility=d?"visible":"hidden",l.style.order=String(-c)})},[s,o]);const a=z(()=>s.slice(o).sort((l,c)=>c.element.compareDocumentPosition(l.element)-l.element.compareDocumentPosition(c.element)),[s,o]);return te(()=>{e.toggleAttribute("has-menu-items",a.length>0)},[a.length]),{setButtonStates:i,menuButtons:a}},dd=e=>{const{active:t=!1}=e,i=fo(!1);Zu({activity:ld,callback:()=>cd(e),check:()=>t&&!e.hasAttribute("hide-actions"),element:()=>e.shadowRoot?.querySelector("#dropdown")},[t]);const{setButtonStates:n,menuButtons:s}=ud(e),r=z(()=>Uc("height"),[]);return ho(()=>{i.current?r(e,t):r(e,t,{duration:0}),i.current=!0},[t]),M` <div id="bar" part="bar">
